...
&emsp;&emsp;本章提及三种表示字符串的结构：散列、平衡树、后缀数组。在此重点介绍**后缀数组**：初始化文本中**指向每个字符(或每个单词)**的指针数组，对其排序就得到一个后缀数组。然后可以遍历它以查找接近的字符串，或使用二分搜索查找单词或词组。

&emsp;&emsp;本章提及一种通过生成k阶马尔科夫链来生成随机文本的方法，代码已经放在对应文件夹中。