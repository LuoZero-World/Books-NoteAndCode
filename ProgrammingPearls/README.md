...
&emsp;&emsp;本章提及三种表示字符串的结构：散列、平衡树、后缀数组。在此重点介绍**后缀数组**：初始化文本中**指向每个字符(或每个单词)** 的指针数组，对其排序就得到一个有序的后缀数组。然后可以遍历它以查找接近的字符串，或使用二分搜索查找单词或词组。

&emsp;&emsp;后缀数组用法诸多，如**查找字符串中出现的最长重复子串**，因为这一重复子串定会在后缀数组中多次出现，因此将后缀数组排好序遍历相邻元素即可；书中另提及一种后缀数组生成随机文本的方法，代码已经放在对应文件夹中。

- 修改查找重复字符串的程序，以找出**重复次数超过M次的最长字符串**？
- 给定两个输入文本，找出两段文本中都出现的最长字符串
